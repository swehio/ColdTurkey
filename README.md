...
#### ▪️ 결정 사항
- `Potion` 클래스 내부에 약의 상태 정보를 모두 포함하도록 설계
...
씬을 이동하는 과정에서 재료 데이터를 참조 복사로 전달하여 추후 재료 데이터를 초기화 시에 Potion의 재료 데이터도 초기화되어
재료 정보가 손실되는 문제가 발생했습니다.

#### ▪️해결 방법
- 재료 배열을 Potion으로 전달할 때 값 복사로 Potion에 전달
- 값 복사한 재료 정보와 그 밖에 Potion에 필요한 능력 사용 여부, 독약 여부 등을 생성자의 인자로 전달하여 깊은 복사

### 2. 이름 변경으로 경로 참조 손실로 인한 씬 삭제 문제
프로젝트 진행 중 파일 이름을 변경한 뒤 Git에 푸시하는 과정에서 파일 이름 변경으로 인해 
기존 경로에 대한 참조가 끊기며 씬(Scene) 파일이 삭제되는 문제가 발생했습니다.

#### ▪️해결 방법
- Git의 커밋 해시(commit hash)를 통해 해당 커밋으로 직접 진입  
- 문제 발생 직전 커밋으로 reset 및 되돌리기 작업 수행
- 씬 파일을 복구한 후, 실수로 올리려던 다이어로그 일부만 다시 생성하여 피해를 최소화
